...
3. В данном окне кликаем по ссылке «Создание учетной записи». Откроется окно, в котором указываем имя учетной записи (для учётной записи ученика я дал имя Student) и тип (обычный пользователь).
