...
После того, как все настройки внесены, **настоятельно** рекомендую [создать точку восстановления системы](https://github.com/auto-no-mous/Clean-school-computer-win/blob/main/restore_point_create.md), т.к. далее мы будем так или иначе вносить изменения в ключи системного реестра. Все дальнейшие действия вы делаете на **свой страх и риск**.
...
net user **"Student"** /PasswordChg:No

но вместо **Student** вписываем имя учетной записи ученика

  ![Запрет смены пароля через командную строку](/images/no_password_change.png "Запрет смены пароля через командную строку")
...
3. Готово!
  ...
  ![Папка автозагрузки](/images/win_10/autorun_folder.png "Папка автозагрузки")

Остаётся только перезагрузить ПК, и проверить что всё работает. Теперь, после входа в учётную запись, должно ненадолго появляться окно терминала - это исполняется наш файл, очищая рабочий стол, и восстанавливая имена значков "Корзина" и "Компьютер".
